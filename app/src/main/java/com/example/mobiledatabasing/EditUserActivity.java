...
public class EditUserActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        userid = intent.getIntExtra(db.TBL_USERS_ID, 0);

        select_user = db.getSelectUser(userid);

        etUsername.setText(select_user.get(0).get(db.TBL_USERS_USERNAME));
        etFullname.setText(select_user.get(0).get(db.TBL_USERS_FULLNAME));
    }
        ...
        switch (item.getItemId()) {
            case R.id.btnSave:
                ...
                if (formsuccess == 2) {
                    ...
                    map_user.put(db.TBL_USERS_ID, String.valueOf(userid));
                    map_user.put(db.TBL_USERS_USERNAME, username);
                    map_user.put(db.TBL_USERS_FULLNAME, fullname);
